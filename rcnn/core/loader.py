...
from rcnn.io.rpn import get_rpn_testbatch, get_rpn_batch, get_rpn_batch_from_recordio,assign_anchor
...
class AnchorLoaderAvaRecordIO(mx.io.DataIter):
    def __init__(self, feat_sym, recordio, classes, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 feat_stride=16, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), allowed_border=0,
                 aspect_grouping=False, use_data_augmentation=False):
        """
        This Iter will provide roi data to Fast R-CNN network
        :param feat_sym: to infer shape of assign_output
        :param recordio: must be preprocessed
        :param batch_size: must divide BATCH_SIZE(128)
        :param shuffle: bool
        :param ctx: list of contexts
        :param work_load_list: list of work load
        ...
        # infer properties from roidb
        self.size = len(list(self.recordio.keys))
        self.index = np.arange(self.size)
    ...
    def get_batch(self):
        ...
        cur_from = self.cur
        cur_to = min(cur_from + self.batch_size, self.size)
        roidb = [self.recordio.read_idx(self.index[i]) for i in range(cur_from, cur_to)]
